...
//!
//! The crate provides the [`WasmerProvider`] type which can be used
...
//! use std::sync::Arc;
//! use wasi_provider::WasmerProvider;
//!
//! async {
...
//!     let provider = WasmerProvider::new(store, &kubelet_config, kubeconfig.clone(), plugin_registry, device_plugin_manager).await.unwrap();
//!
...
/// WasmerProvider provides a Kubelet runtime implementation that executes WASM
...
type PodHandleMap =
    Arc<RwLock<HashMap<PodKey, Arc<Handle<Runtime, wasmer_runtime::HandleFactory>>>>>;
...
impl WasmerProvider {
    /// Create a new wasmer provider from a module store and a kubelet config
    pub async fn new(
        store: Arc<dyn Store + Sync + Send>,
...
impl Provider for WasmerProvider {
...
impl GenericProvider for WasmerProvider {
    type ProviderState = ProviderState;
    type PodState = PodState;
