...
impl<P: GenericProvider> State<P::PodState> for Allocated<P> {
    async fn next(
        ...
        let pod = pod.latest();
        debug!("Preparing to allocate resources for the pod: {}", pod.name());
        let (client, device_plugin_manager) = {
            let state_reader = provider_state.read().await;
            (
                state_reader.client(),
                state_reader.device_plugin_manager(),
            )
        };
        ...
        if let Some(device_plugin_manager) = device_plugin_manager {
            ...
            for container in pod.all_containers() {
                if let Some(resources) = container.resources() {
                    if let Some(requests) = &resources.requests {
                        let extended_resources: HashMap<String, Quantity> = requests.clone().into_iter().filter(|(resource_name, _)| {
            ...
            if let Err(e) = device_plugin_manager.do_allocate(&pod, container_devices).await {
                    error!("{:?}", e);
            ...
            if let Some(container_allocate_responses) = device_plugin_manager.pod_devices.get_pod_allocate_responses(pod.pod_uid()) {
                let mut host_paths: Vec<String> = Vec::new();
                container_allocate_responses.iter().for_each(|alloc_resp| alloc_resp.mounts.iter().for_each(|m| host_paths.push(m.host_path.clone())));
                // TODO: determine what to name each volume. Currently using whatever the default is.
                let volumes: HashMap<String, VolumeRef> = host_paths.iter_mut().map(|p| HostPathVolumeSource{path: p.clone(), ..Default::default()}).map(|h| KubeVolume { host_path: Some(h), ..Default::default()}).map(|k| (k.name.clone(), VolumeRef::HostPath(HostPathVolume::new(&k).unwrap()))).collect();
