...
use crate::device_plugin_api::v1beta1::{ContainerAllocateResponse, ContainerAllocateRequest};
...
impl PodDevices {
    pub fn new(client: kube::Client) -> Self {
        PodDevices {allocated_devices: Arc::new(Mutex::new(HashMap::new())), client}
    }
    ...
    pub async fn get_active_pods(&self) -> anyhow::Result<HashSet<String>> {
        ...
        Ok(pods.iter().map(|pod| pod.metadata.uid.clone().expect("Pod uid should always be set but was not")).collect::<HashSet<String>>())
    }

    /// get_pods returns the UIDs of all the Pods in the `PodDevices` map
    pub fn get_pods(&self) -> HashSet<String> {
        self.allocated_devices.lock().unwrap().iter().map(|(p_uid, _container_map)| p_uid.clone()).collect::<HashSet<String>>()
    }
    ...
    pub fn remove_pods(&self, pods_to_remove: HashSet<String>) -> anyhow::Result<()> {
        ...
        pods_to_remove.iter().for_each(|p_uid| {allocated_devices.remove(p_uid);});
        Ok(())
    }

    /// get_pods returns the UIDs of all the Pods in the `PodDevices` map
    pub fn get_container_devices(&self, resource_name: &str, pod_uid: &str, container_name: &str) -> Option<HashSet<String>> {
    ...
    pub fn get_allocated_devices(&self) -> DeviceIdMap {
        ...
        allocated_devices.iter().for_each(|(p_uid, container_map)| {
            container_map.iter().for_each(|(container_name, container_devs)| {
                container_devs.iter().for_each(|(resource_name, device_allocate_info)| {
                    if let Some(device_ids) = res.get(resource_name) {
                        res.insert(resource_name.clone(), device_ids.union(&device_allocate_info.device_ids).into_iter().cloned().collect());
                    } else {
                        res.insert(resource_name.clone(), device_allocate_info.device_ids.clone());
                    }
                });
            });
        });
    ...
    pub fn get_pod_allocate_responses(&self, pod_uid: &str) -> Option<Vec<ContainerAllocateResponse>> {
        match self.allocated_devices.lock().unwrap().get(pod_uid) {
            Some(container_devices) => {
                let mut container_allocate_responses = Vec::new();
                container_devices.iter().for_each(|(container_name, resource_allocate_info)| {
                    resource_allocate_info.iter().for_each(|(resource_name, dev_info)| {
                        container_allocate_responses.push(dev_info.allocate_response.clone());
                    });
                });
