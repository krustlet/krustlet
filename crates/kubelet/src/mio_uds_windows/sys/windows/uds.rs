...
use super::selector::ReadyBinding;
use crate::mio_uds_windows::net::{
    self, AcceptAddrsBuf, SocketAddr, UnixListenerExt, UnixStreamExt,
};
...
impl UnixStream {
    fn new(socket: net::UnixStream, deferred_connect: Option<SocketAddr>) -> UnixStream {
        UnixStream {
    ...
    pub fn connect(socket: net::UnixStream, addr: &SocketAddr) -> io::Result<UnixStream> {
...
impl UnixListener {
    pub fn new(socket: net::UnixListener) -> io::Result<UnixListener> {
...
impl ListenerImp {
    ...
    fn schedule_accept(&self, me: &mut ListenerInner) {
        ...
        let res = Socket::new()
            .map(|sock| unsafe { net::UnixStream::from_raw_socket(sock.into_raw_socket()) })
            .and_then(|sock| {
                unsafe {
