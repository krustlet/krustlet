...
$ krustlet-wasi --node-ip $mainIP --node-name krustlet --tls-cert-file=~/.krustlet/config/krustlet.crt --tls-private-key-file=~/.krustlet/config/krustlet.key --bootstrap-file=~/.krustlet/config/bootstrap.conf
```
